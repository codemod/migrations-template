...
function getMetricFilePath(filename: string): string {
  ...
  const testsIndex = stripped.lastIndexOf('/tests/')
  if (testsIndex !== -1) {
    return stripped.slice(testsIndex + 1)
  }

  const normalized = path.normalize(stripped)
  const relativePath = path.isAbsolute(normalized) ? path.relative(process.cwd(), normalized) : normalized
...
function isReactComponent(node: SgNode<TSX>): boolean {
  if (node.is('function_declaration')) {
    return node.has({ rule: { kind: 'jsx_element' } }) || node.has({ rule: { kind: 'jsx_self_closing_element' } })
  }
...
function hasClosureDependency(innerComponent: SgNode<TSX>, outerComponent: SgNode<TSX>, innerName: string): boolean {
